...
firebase.init({
  onAuthStateChanged: function(data) {
    ...
    if (data.loggedIn) {
      console.log(
        "user's email address: " + (data.user.email ? data.user.email : "N/A")
      );
    }
  }
});
...
firebase
  .fetchProvidersForEmail(emailAddress)
...
firebaseWebApi
  .auth()
  .fetchProvidersForEmail(user.email)
  .then(result =>
    console.log(`Providers for ${user.email}: ${JSON.stringify(result)}`)
  )
...
firebase
  ...
  .then((methods: Array<string>) => {
    console.log(
      `Sign-in methods for ${emailAddress}: ${JSON.stringify(methods)}`
    );
  });
```
...
if (!user || !user.email) {
  console.log(
    "Can't fetch sign-in methods; no user with an emailaddress logged in."
  );
  return;
}
...
firebaseWebApi
  ...
  .then(result =>
    console.log(`Sign-in methods for ${user.email}: ${JSON.stringify(result)}`)
  )
  .catch(error =>
    console.log("Fetch Sign-in methods for Email error: " + error)
  );
```
...
firebaseWebApi
  .auth()
  .signInWithEmailAndPassword("eddy@x-services.nl", "firebase")
  .then(() => console.log("User logged in"))
  .catch(err => console.log("Login error: " + JSON.stringify(err)));
```
...
firebaseWebApi
  .auth()
  .sendSignInLinkToEmail("eddy@x-services.nl", {
    url: "https://domain.com?foo=bar",
    // the stuff below is optional, if not set the plugin will infer this for you (bundle/package is taken from currently used platform)
    iOS: {
      bundleId: "my.bundle.id"
...
firebaseWebApi
  .auth()
  .createUserWithEmailAndPassword("eddyverbruggen@gmail.com", "firebase")
  .then((user: User) => {
...
_NOTE:_ iOS 10 and below issue for Google Auth when opening from a modal.
